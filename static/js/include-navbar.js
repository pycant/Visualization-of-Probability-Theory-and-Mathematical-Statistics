...
(function () {
  ...
  function applyActiveLink(root) {
    var slug =
      (window.NAV_CONFIG && window.NAV_CONFIG.active) || slugFromPath();
  ...
  function positionDropdown(root) {
    var dropdown =
      document.querySelector("#chapters-dropdown") ||
      root.querySelector("#chapters-dropdown");
    if (!dropdown) return;
    try {
      var nav =
  ...
  function hoistDropdownToBody(root) {
    try {
      ...
      dlog("hoistDropdownToBody error", e && e.message);
    }
  }
  ...
  function wireDropdown(root) {
    ...
    var dropdown =
      ...
      root.querySelector("#chapters-dropdown");
    dlog(
      "wireDropdown start toggleExists=",
      !!toggle,
      "dropdownExists=",
      !!dropdown
    );
  ...
  function bindChapterCardInteractions() {
    try {
      ...
      dlog("bindChapterCardInteractions error", e && e.message);
    }
  }
  ...
  function loadNavbar() {
    ...
    fetch(partialUrl, { cache: "no-cache" })
      ...
      .then(function (html) {
        ...
        var navEl = temp.querySelector("nav") || temp.firstElementChild;
